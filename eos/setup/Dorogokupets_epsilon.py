...
rho0 = 1.0/6.8175 #molcm-3 OR 1.0/49.026 #A-3 
...
def Dorog_Cv(rho,T):
    theta = theta_func(rho ,theta0)
    Cv = Cvm * (theta/T)**2 * np.exp(theta/T)/(np.exp(theta/T)-1)**2
...
def EoSthings(P,T):
    ...
    for i in range(len(P)):
        ...
        print(T , rho_min/rho0 , rho_max/rho0 , P[i] , Dorog_P_fixT(rho_min) , Dorog_P_fixT(rho_max))
